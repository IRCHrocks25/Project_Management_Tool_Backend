...
export class TasksService {
    ...
    if (!savedTask.assignedToId && savedTask.type && savedTask.projectId) {
      try {
        ...
        if (roles && roles.length > 0) {
          ...
          const headPMData = {
            type: NotificationType.TASK_AVAILABLE,
            title: 'New task available',
            message: `A new "${savedTask.title}" task is available for ${project?.clientName || 'Unknown Project'}. No one is assigned yet.`,
            projectId: savedTask.projectId,
            taskId: savedTask.id,
  ...
  /**
   * Fetch all task conversations across all tasks, with task and project context.
   * Used for the Forum / global conversations page.
   */
